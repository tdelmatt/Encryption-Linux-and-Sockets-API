 ...
 *Assignment 4
 otp_dec_d.c
 *Operating Systems
 *This component recieves an encrypted file and a keyfile
 * from otp_dec.c it decripts the file and writes the result
 * back to otp_dec.c
 * ******************************************/
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[])
{
	...
	char buffer[50000]; //definitions...
	struct sockaddr_in serv_addr, cli_addr, serv_addr1, cli_addr1; //definitions...
	char portstr[15];
	pid_t cpid;
	//int n;
	if (argc < 2) {
	...
	while(ischild == 0){	
		...
		//if in child, process data, write data, quit
			...
			wrdata("ddddddddd", newsockfd);
			if(verifyised[0] == 'e'){
				printf("this is enc, connection not allowed\n");
				continue;
			}
			...
			//client connects to new portnumber
			...
			//decrypt data and write to dec
			decrypt(buffer, keyin);
			wrdata(buffer, newsockfd);
		}	
		else{//IN PARENT		
				
		}
		//if in child set ischild to 1, close fds
